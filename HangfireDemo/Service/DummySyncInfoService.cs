...
namespace Service
{
    public class DummySyncInfoService : IDummySyncInfoService
    {
        ...
        public async Task<int> UpdateLastSyncId(int Id, long lastSyncId)
        {
            var sql = @"UPDATE dummy_sync_info 
                        SET last_sync_id=@last_sync_id
                        WHERE id=@id";
        ...
        public async Task<int> UpdateLastSyncTime(int Id, DateTime time)
        {
            var sql = @"UPDATE dummy_sync_info 
        ...
        public async Task<int> UpdateStatus(int status)
        {
            var sql = @"UPDATE dummy_sync_info 
                        SET status=@status
                        WHERE 1=1";
